...
function sys=mdlDerivatives(t,x,u,P)
  ...
  wn     = u(4); % wind in North
  we     = u(5); % wind in East
  
  % Compute chi_c_dot from roll feedforward (Assume no feedforward)
  chi_c_dot = 0;
  ...
  % Implement condition to ensure turn rate does not exceed 
  % g/V*tan(P.phi_max), otherwise, chiddot = 0....

    hdot_c = 0;
  hddot = P.b_hdot*(hdot_c-hdot) + P.b_h*(h_c-h);
  % Implement condition to ensure climb rate does not exceed 
  % Va*sin(P.gamma_max), otherwise, hddot = 0....
...
function sys=mdlOutputs(t,x,u,P)
  ...
  hdot   = x(6); % climb rate
  Va     = x(7); % airspeed
  wn     = 0; % wind in North (Assumed to be 0)
  we     = 0; % wind in East (Assumed to be 0)
  
  alpha  = 0;
  beta   = 0;
  
  % Solve for heading and groundspeed
  psi   = chi - asin( (-wn*sin(chi)+we*cos(chi))/Va );
  Vg    = [cos(chi), sin(chi)]*(Va*[cos(psi); sin(psi)] + [wn; we]);
  
  % Roll angle is given by psidot = g/V*tan(phi)
  phi   = atan(Vg*chidot/P.gravity);
  
  % Theta equal flight path angle given by hdot = V sin(gamma)
  theta = asin(hdot/Va);
...
sys = real([pn; pe; h; Va; alpha; beta; phi; theta; chi; p; q; r; Vg; wn; we; psi;...

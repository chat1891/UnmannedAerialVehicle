...
%    delta_t     = uu(53);            % throttle setting (unitless)
  ...
  % first time function is called, initialize plot and persistent vars
    if t==0,
        ...
        subplot(3,1,1)
        hold on
        p_handle = graph_y_yhat_yd(t, p, p_hat, 0, 'p', []);
        subplot(3,1,2)
        hold on
        q_handle = graph_y_yhat_yd(t, q, q_hat, 0, 'q', []);
        subplot(3,1,3)
        hold on
    ...
    % at every other time step, redraw state variables
       ...
       %% submission 
       graph_y_yhat_yd(t, p, p_hat, 0, 'p', p_handle);
       graph_y_yhat_yd(t, q, q_hat, 0, 'q', q_handle);
